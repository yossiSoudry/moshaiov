...
function LoginContent() {
  ...
  const handleOAuth = async (provider: 'GOOGLE' | 'FACEBOOK' | 'GITHUB') => {
    ...
    try {
      ...
      const { authorizationUrl } = await omni.getOAuthAuthorizeUrl(provider, { redirectUrl });
      window.location.href = authorizationUrl;
    } catch (err) {
      console.error('OAuth error:', err);
      setOauthError(err instanceof Error ? err.message : 'התחברות דרך Google לא זמינה כרגע');
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 pt-24 pb-16">
      ...
      >
        ...
        <div className="bg-background border border-border rounded-xl p-6 lg:p-8">
          {/* OAuth buttons - always show Google */}
          <div className="space-y-3">
            ...
            {oauthProviders.includes('facebook') && (
              <Button
                ...
                disabled={isOAuthLoading}
              >
                <svg className="h-5 w-5 ms-2" fill="#1877F2" viewBox="0 0 24 24">
